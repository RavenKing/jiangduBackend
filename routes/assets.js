...
async function getAssetTag(data) {
  ...
  const result = await policyTag.raw(
    'select "TAG_ID", "TAG_NAME","TAG_VALUE" ,"TAG_CATEGORY" from "SAP_JIANGDU_TAGS" where "TAG_ID" in (Select "TAG_ID_TAG_ID" from "SAP_JIANGDU_TAG_AssetS" where "ASSET_ID_ASSET_ID" = \'' +
      data.ASSET_ID +
      "' )"
  );
  return result;
