...
router.get('/latest/policy/companies', async(req, res, next)=>{
    console.log("working")
    ...
    const result = await histo.raw(
        "SELECT A1.USER_ID, A1.COMPANY_NAME, A2.COUNT FROM \"SAP_JIANGDU_FREQUENTLY_USED_ENTERPRISES\" as A2 LEFT JOIN \"SAP_JIANGDU_USERS\" as A1 ON A2.ENTERPRISE_ID_USER_ID = A1.USER_ID"
    )
    res.send(result); 
})
